...
public class StringUtils {
	...
	for(int i=0; i<str.length(); i++)
	{
		...
		while(!(str.charAt(i)==','))
		{
		    if(str.charAt(i)== '"'){ //removes the beginning quote from a quoted section
			count=0;
			if(str.charAt(i+1)=='"') /**!!!!!!!!!!*/
			{
		    ...
		    count++; //makes sure all parts before the last quote are added
		    i++;
		}//while
		if (str.charAt(0) == '"') //removes the beginning quote from a quoted section at 0
		    count--;
		num++; //removes the quote at the end of a quoted section
	    }//if
	    else if (str.charAt(i) == seperator) 
		...
		num=0; //makes sure that the parts without quotes don't remove the extra element where a " would be 
		
	    }//else if(str==sep) 
	    else if (i == str.length() - 1) { //accounts for the last element that is added
		    newArray[k] = str.substring(i - count + 1);
		}//else if
	    count++;
	}//for
